...
import type { ConsultarAuxilioEmergencialPorMunicipioInput, ConsultarAuxilioEmergencialPorMunicipioOutput, BeneficioPorMunicipioDTO } from "./types";
import { Heart } from "lucide-react";

export function renderConsultarAuxilioEmergencialPorMunicipioInput(input: ConsultarAuxilioEmergencialPorMunicipioInput) {
  return (
...
export function renderConsultarAuxilioEmergencialPorMunicipioOutput(output: ConsultarAuxilioEmergencialPorMunicipioOutput | string) {
  const beneficios = parseToArray<BeneficioPorMunicipioDTO>(output);
  if (beneficios.length === 0) return <EmptyState message="Nenhum benefício encontrado" icon={<Heart className="w-4 h-4" />} />;
