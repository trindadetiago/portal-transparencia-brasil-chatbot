...
import type { ConsultarPepsInput, ConsultarPepsOutput, PEPDTO } from "./types";

export function renderConsultarPepsInput(input: ConsultarPepsInput) {
  return <InfoField label="Consulta" value={JSON.stringify(input)} />;
}

export function renderConsultarPepsOutput(output: ConsultarPepsOutput | string) {
  const items = parseToArray<PEPDTO>(output);
