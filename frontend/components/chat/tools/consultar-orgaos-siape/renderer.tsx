...
import type { ConsultarOrgaosSiapeInput, ConsultarOrgaosSiapeOutput, CodigoDescricaoDTO } from "./types";
import { Building2 } from "lucide-react";

export function renderConsultarOrgaosSiapeInput(input: ConsultarOrgaosSiapeInput) {
  return input.descricao ? <StatusBadge status={`Busca: ${input.descricao}`} variant="info" /> : null;
}

export function renderConsultarOrgaosSiapeOutput(output: ConsultarOrgaosSiapeOutput | string) {
